...
public class OmniDrive extends SubsystemBase
{
    ...
    public OmniDrive() {
        ...
        for (int i=0; i<Constants.MOTOR_NUM; i++) {
            pidControllers[i] = new PIDController(0.0,2,0);
        }
    ...
    public void setPIDSpeed012(double speed0, double speed1, double speed2, double speedW)
    {
        
        pidInputs[0] = speed0; //+ve is CW. Need to reverse
        pidInputs[1] = speed1;
        pidInputs[2] = speed2;
        pidInputW = speedW;
    }
    ...
    public void doPID( ){
        for (int i=0; i<Constants.MOTOR_NUM; i++) {
            ...
            encoderSpeeds[i] = (encoderDists[i]-encoderDists_2[i])/0.02;//encoders[i].getPeriod();
